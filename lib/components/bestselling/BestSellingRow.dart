...
class BestSellingRow extends StatefulWidget {
  final title;
  BestSellingRow({this.title});

  @override
  _BestSellingRowState createState() => _BestSellingRowState();
}

class _BestSellingRowState extends State<BestSellingRow> {
  ...
  void getBestSelling() async {
    if (await con.check() == true) {
      try {
        final response = await http.post("${curl}Api/getBestSelling", body: {
          "number": this.number,
          "valid": this.valid,
        });
